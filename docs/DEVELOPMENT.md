...
**demo.4.1 发布记录：** 2026-09-16 云端 3 片共 84 项回归全部通过，9 项服务 / 启动器和 3 项美术测试、实际 ZIP 验证与公开附件校验通过，详见 [发布验证记录](RELEASE_VERIFICATION.md)。

**本地分段记录：** 84 项回归均有通过结果（完整干净源码先通过 83 项；帧率计时修正后 5 项性能专项通过），9 项服务 / 启动器与 3 项美术 Node 测试通过。初始完整命令的失败仍保留在 `dist/demo4-1-release-clean.log`，不把分段复验写成单次全绿；云端三个分片仍检查全部 84 项。
